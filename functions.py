...
def select_planilha_acqua(data):
    mydb.connect()
    cursor.execute("SELECT c.id, c.nome, c.telefone, v.nome, r.tipo, r.fotos, from reserva as r  JOIN cliente as c on r.id_cliente = c.id JOIN vendedores as v ON r.id_vendedor = v.id where r.data = %s ", (data,))
